...
# servicio para listar  los registros
    def list_rutas(self, skip: int, limit: int):
        return self.db.query(Ruta).filter(Ruta.activo == True).offset(skip).limit(limit).all()
    def count_rutas(self):
        return self.db.query(Ruta).filter(Ruta.activo == True).count()
    
    
    # servicio para crear un registro
    async def create_rutas(self, payload: RutaCreate, 
                            request: Request, tokenpayload: dict):
        datacreate = self.db.query(Ruta).filter(
            Ruta.nombre == payload.nombre,
                Ruta.activo == True).first()
        if datacreate:
        ...
        entity = Ruta(nombre=payload.nombre, id_persona=tokenpayload.get("sub"), 
    ...
    async def show(self, ruta_id: int):
        entity = self.db.query(Ruta).filter(
            Ruta.id == ruta_id,
                Ruta.activo == True).first()
        if not entity:
    ...
    async def update_rutas(self, ruta_id: int, 
                            payload: RutaUpdate, 
                            request: Request, tokenpayload: dict):
        dataupdate = self.db.query(Ruta).filter(
            Ruta.id == ruta_id,
                Ruta.activo == True).first()
        if payload.nombre:
            existe = (
                self.db.query(Ruta)
                .filter(Ruta.nombre == payload.nombre, Ruta.id != ruta_id)
                .first()
            )
            if existe:
    ...
    async def delete_ruta(self, ruta_id: int, request: Request, tokenpayload: dict):
        datadelete = self.db.query(Ruta).filter(
            Ruta.id == ruta_id,
                Ruta.activo == True).first()
        if not datadelete:
            return HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="La ruta no fue hallada")
